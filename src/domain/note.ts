...
export const NATUTAL_PITCH_INDICES: PitchIndex[] = [
    0, 2, 4, 5, 7, 9, 11
]
...
export function fromMidi(midi: MidiValue): Note {
    ...
    const octave = (Math.floor(pianoRage / SEMITONES_IN_OCTAVE) + 1 ) as OctaveIndex
    const index = pianoRage % SEMITONES_IN_OCTAVE
    const pitch = PITCHES_REGISTRY[index]

    const isSharp = !NATUTAL_PITCH_INDICES.includes(index)
